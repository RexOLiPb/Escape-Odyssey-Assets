...
namespace Platformer
{
    public class PlayerCombat : Combat, ICombat
    {
        ...
        public override void Start()
        {
            ...
            StartCoroutine(HPLossOverTime());
        }

        IEnumerator HPLossOverTime()
        {
            while (true) // 持续循环
            {
                ...
                // 这里是减少HP的逻辑
                // 假设Stats对象是playerController的一部分
                if (playerController.stats.statsData.HP > 0 && SceneManager.GetActiveScene().name == "GameScene")
                {
                    //Debug.Log("Active scene:" + SceneManager.GetActiveScene().name); 
                    playerController.stats.statsData.HP -= 4; // 每秒减少1点HP
                    UIManager.Instance.UpdateHP(playerController.stats.statsData.HP);
                }
            }
        }
